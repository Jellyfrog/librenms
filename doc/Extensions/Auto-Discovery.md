...
network. We can do this with some methods, which are explained
...
Note: you must add a minimum of ONE device before
...
The system tries these details when it adds devices. You can specify
...
To add devices, we must know your subnets. Then we do not go
...
it that you cannot add automatically, you can exclude this as follows:
...
By default, we do not add devices by IP address. We look for a reverse
dns name to be found and add with that. If this fails
and you want to add devices automatically, you
need to set `$config['discovery_by_ip'] = true;`

### Short hostnames

If your devices only return a short hostname such as lax-fa0-dc01 but
the full name must be lax-fa0-dc01.example.com, you can
set
...
devices with duplicate sysNames, set
...
By default, SNMP Scan scans `nets`, and it obeys `autodiscovery.nets-exclude`.
...
The system adds new discovered devices to the `default_poller_group`. If not set, this value is 0.
