...
test('simple', function () {
    ...
    expect($response->values())->toEqual(['IF-MIB::ifDescr[1]' => 'lo', 'IF-MIB::ifDescr[2]' => 'enp4s0']);
    expect($response->value())->toEqual('lo');
    expect($response->table())->toEqual(['IF-MIB::ifDescr' => [1 => 'lo', 2 => 'enp4s0']]);
    expect($response->table(1))->toEqual([1 => ['IF-MIB::ifDescr' => 'lo'], 2 => ['IF-MIB::ifDescr' => 'enp4s0']]);

    // snmptranslate type response
    $response = new SnmpResponse("IF-MIB::ifDescr\n");

    expect($response->isValid())->toBeTrue();
    expect($response->values())->toEqual(['' => 'IF-MIB::ifDescr']);
    expect($response->value())->toEqual('IF-MIB::ifDescr');
    expect($response->table())->toEqual(['' => 'IF-MIB::ifDescr']);
    ...
    expect($response->isValid())->toBeTrue();
    expect($response->value())->toEqual('default');
    ...
    expect($response->table())->toEqual(['Q-BRIDGE-MIB::dot1qVlanStaticName' => [
        1 => 'default',
        6 => '\\single\\',
        9 => '\\\\double\\\\',
    ]]);

    LibrenmsConfig::set('snmp.unescape', true);
    // for buggy versions of net-snmp
    $response = new SnmpResponse("Q-BRIDGE-MIB::dot1qVlanStaticName[1] = \"\\default\\\"\nQ-BRIDGE-MIB::dot1qVlanStaticName[6] = \\single\\\nQ-BRIDGE-MIB::dot1qVlanStaticName[9] = \\\\double\\\\\n");
    expect($response->values())->toEqual([
        'Q-BRIDGE-MIB::dot1qVlanStaticName[1]' => 'default',
        'Q-BRIDGE-MIB::dot1qVlanStaticName[6]' => 'single',
...
test('value fetching', function () {
    ...
    expect($response->value())->toEqual('lo');
    expect($response->value('IF-MIB::ifDescr[1]'))->toEqual('lo');
    expect($response->value('IF-MIB::ifDescr[2]'))->toEqual('enp4s0');
    expect($response->value('IF-MIB::ifDescr.2'))->toEqual('enp4s0');
    expect($response->value(['IF-MIB::ifDescr[1]', 'IF-MIB::ifDescr[2]']))->toEqual('lo');
    expect($response->value(['IF-MIB::ifName[2]', 'IF-MIB::ifDescr[2]', 'IF-MIB::ifDescr[1]']))->toEqual('enp4s0');
    expect($response->value(['IF-MIB::ifDescr.1', 'IF-MIB::ifDescr.2']))->toEqual('lo');

    expect($response->value('IF-MIB::ifDescr'))->toEqual('lo');
    expect($response->value('IF-MIB::ifAlias'))->toEqual('alias one');
    expect($response->value('ifAlias'))->toEqual('');
    expect($response->value('IF-MIB:'))->toEqual('');
    expect($response->value('IF-MIB::ifA'))->toEqual('');

    $response = new SnmpResponse("ifName.1 = lo\nifAlias.3 = cust42\nifAlias.4 = cust51\n\n");
    expect($response->value('ifName'))->toEqual('lo');
    expect($response->value('ifName'))->toEqual('lo');
    expect($response->value('ifAlias'))->toEqual('cust42');
    expect($response->value('ifAlias.4'))->toEqual('cust51');
...
test('empty values', function () {
    ...
    expect($response->value('IF-MIB::ifAlias[1]'))->toEqual('');
    expect($response->value('IF-MIB::ifAlias[2]'))->toEqual('0');
    expect($response->value('IF-MIB::ifAlias[3]'))->toEqual('');
...
test('values by index', function () {
    ...
    expect($response->isValid())->toBeTrue();
    expect($response->valuesByIndex())->toEqual([
        1 => [
            'IF-MIB::ifIndex' => '1',
            'IF-MIB::ifDescr' => 'lo',
        ],
    ...
    expect($response->valuesByIndex($existing))->toEqual([
        0 => 'first',
        1 => [
            'IF-MIB::ifIndex' => '1',
            'IF-MIB::ifDescr' => 'lo',
...
test('group by index', function () {
    ...
    expect($response->groupByIndex(3))->toEqual(['1.10.1' => ['.1.3.6.1.2.1.2.2.1.10.1' => 495813425], '1.10.2' => ['.1.3.6.1.2.1.2.2.1.10.2' => 3495809228]]);
    expect($response->groupByIndex(-2))->toEqual(['6.1.2.1.2.2.1.10.1' => ['.1.3.6.1.2.1.2.2.1.10.1' => 495813425], '6.1.2.1.2.2.1.10.2' => ['.1.3.6.1.2.1.2.2.1.10.2' => 3495809228]]);

    $response = new SnmpResponse(".1.3.6.1.2.1.2.2.1.10.1 = 495813425\n.1.3.6.1.2.1.2.2.1.11.1 = 3495809228\n");
    expect($response->groupByIndex())->toEqual([1 => ['.1.3.6.1.2.1.2.2.1.10.1' => 495813425, '.1.3.6.1.2.1.2.2.1.11.1' => 3495809228]]);
    expect($response->groupByIndex(2))->toEqual(['10.1' => ['.1.3.6.1.2.1.2.2.1.10.1' => 495813425], '11.1' => ['.1.3.6.1.2.1.2.2.1.11.1' => 3495809228]]);
    expect($response->groupByIndex(-5))->toEqual(['1.2.2.1.10.1' => ['.1.3.6.1.2.1.2.2.1.10.1' => 495813425], '1.2.2.1.11.1' => ['.1.3.6.1.2.1.2.2.1.11.1' => 3495809228]]);

    $response = new SnmpResponse("SOME-MIB::oid.1.1.0 = 14\nSOME-MIB::oid.1.2.0 = 42\n");
    expect($response->isValid())->toBeTrue();
    expect($response->groupByIndex())->toEqual([0 => ['SOME-MIB::oid.1.1.0' => '14', 'SOME-MIB::oid.1.2.0' => '42']]);
    expect($response->groupByIndex(2))->toEqual(['1.0' => ['SOME-MIB::oid.1.1.0' => '14'], '2.0' => ['SOME-MIB::oid.1.2.0' => '42']]);
...
test('multi line', function () {
    ...
    expect($response->isValid())->toBeTrue();
    expect($response->value())->toEqual("something\n on two lines");
    expect($response->values())->toEqual(['SNMPv2-MIB::sysDescr.1' => "something\n on two lines"]);
    expect($response->table())->toEqual(['SNMPv2-MIB::sysDescr' => [1 => "something\n on two lines"]]);
});

function numericTest(): void
{
    $response = new SnmpResponse(".1.3.6.1.2.1.2.2.1.10.1 = 495813425\n.1.3.6.1.2.1.2.2.1.10.2 = 3495809228\n");

    expect($response->isValid())->toBeTrue();
    expect($response->value())->toEqual('496255256');
    expect($response->values())->toEqual(['.1.3.6.1.2.1.2.2.1.10.1' => '496255256', '.1.3.6.1.2.1.2.2.1.10.2' => '3495809228']);
    expect($response->table())->toEqual(['.1.3.6.1.2.1.2.2.1.10.1' => '496255256', '.1.3.6.1.2.1.2.2.1.10.2' => '3495809228']);
    expect($response->table(3))->toEqual(['.1.3.6.1.2.1.2.2.1.10.1' => '496255256', '.1.3.6.1.2.1.2.2.1.10.2' => '3495809228']);
}

function tableTest(): void
{
...
');
    ...
    expect($response->isValid())->toBeTrue();
    expect($response->value())->toEqual('34');
    expect($response->values())->toEqual([
    ...
    expect($response->table(1))->toEqual([
...
function trimTest(): void
{
    $response = new SnmpResponse(".1.3.6.1.2.1.2.2.1.10.1 = \\\"4958\\\"\n.1.3.6.1.2.1.2.2.1.10.2 = \"\" 349\r\n\n");
    expect($response->isValid())->toBeTrue();
    expect($response->value())->toEqual('4958');
    expect($response->values())->toEqual(['.1.3.6.1.2.1.2.2.1.10.1' => '4958', '.1.3.6.1.2.1.2.2.1.10.2' => '349']);
...
test('error handling', function () {
    ...
    expect($response->getErrorMessage())->toEqual('Timeout: No Response from udp:127.1.6.1:1161.');
    ...
    expect($response->isValid())->toBeTrue();
    expect($response->getErrorMessage())->toEqual('');
    expect($response->values())->toEqual(['SNMPv2-MIB::sysObjectID.0' => 'wrong thing']);
    ...
    expect($response->isValid())->toBeFalse();
    expect($response->getErrorMessage())->toEqual('No more variables left in this MIB View (It is past the end of the MIB tree)');
    ...
    expect($response->isValid())->toBeFalse();
    expect($response->getErrorMessage())->toEqual('No Such Instance currently exists at this OID.');

    // Unknown user name
    $response = new SnmpResponse('', "snmpget: Unknown user name (Sub-id not found: (top) -> sysDescr)\n", 1);
    expect($response->isValid())->toBeFalse();
    expect($response->getErrorMessage())->toEqual('Unknown user name');
    ...
    expect($response->isValid())->toBeFalse();
    expect($response->getErrorMessage())->toEqual('Authentication failure');

    // OID not increasing
    $response = new SnmpResponse(".1.3.6.1.2.1.2.2.1.1.1 = INTEGER: 1\n", "Error: OID not increasing: .1.3.6.1.2.100.2.2.1.1\n >= .1.3.6.1.2.1.2.2.1.1.1\n", 1);
    expect($response->isValid())->toBeFalse();
    expect($response->getErrorMessage())->toEqual('Error: OID not increasing: .1.3.6.1.2.100.2.2.1.1');

    // NULL return
    $response = new SnmpResponse("hrDeviceTable = NULL\n", '', 0);
    expect($response->isValid())->toBeTrue();
    expect($response->getRawWithoutBadLines())->toEqual('');
