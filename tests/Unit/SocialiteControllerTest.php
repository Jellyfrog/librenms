...
test('set roles from claim okta admin', function () {
    ...
    $result = runSetRolesFromClaimTest('okta', $rawAttributes, ['admin'], [
...
test('set roles from claim okta global read', function () {
    ...
    $result = runSetRolesFromClaimTest('okta', $rawAttributes, ['global-read'], [
        'Example-Admin-Group' => ['roles' => ['admin']],
        'Example-ReadOnly-Group' => ['roles' => ['global-read']],
    ]);
    expect($result)->toBeTrue();
...
test('set roles from claim saml2 admin', function () {
    ...
    $result = runSetRolesFromClaimTest('saml2', [$attr], ['admin'], [
...
test('set roles from claim saml2 global read', function () {
    ...
    $result = runSetRolesFromClaimTest('saml2', [$attr], ['global-read'], [
        'G_librenms_admins' => ['roles' => ['admin']],
        'G_librenms_users' => ['roles' => ['global-read']],
    ]);
    expect($result)->toBeTrue();
