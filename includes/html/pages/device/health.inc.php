...
$mempools = dbFetchCell('select count(*) from mempools WHERE device_id = ?', [$device['device_id']]) + count_mib_mempools($device);
$processor = dbFetchCell('select count(*) from processors WHERE device_id = ?', [$device['device_id']]) + count_mib_processors($device);
