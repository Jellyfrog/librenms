...
if (in_array($device['os'], ['linux', 'endian', 'proxmox', 'recoveryos'])) {
    ...
    [,,$version,,,$features] = explode(' ', $device['sysDescr']);
    $features = str_replace('(', '', $features);
    $features = str_replace(')', '', $features);
    [,,$hardware] = explode("$features", $device['sysDescr']);
} elseif ($device['os'] == 'solaris' || $device['os'] == 'opensolaris') {
    [,,$version,$features,$hardware] = explode(' ', $device['sysDescr']);
    $features = str_replace('(', '', $features);
    $features = str_replace(')', '', $features);
} elseif ($device['os'] == 'monowall' || $device['os'] == 'Voswall') {
    [,,$version,$hardware,$freebsda, $freebsdb, $arch] = explode(' ', $device['sysDescr']);
    ...
    if ($aix_descr[1]) {
        ...
        [$version,$aix_version_min] = array_map('intval', explode('.', $aix_long_version));
    // AIX net-snmp
    } else {
        [,,$aix_version_min,$version,$serial] = explode(' ', $aix_descr[0]);
    }
