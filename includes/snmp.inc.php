 ...
 */
function snmp_mib_parse($oid, $mib, $module, $mibdir = null, $device = [])
{
    $fulloid = explode('.', $oid);
...
function snmp_mib_walk($mib, $module, $mibdir = null, $device = [])
{
 ...
 */
function snmp_mib_load($mib, $module, $included_by, $mibdir = null, $device = [])
{
    $mibs = [];
    ...
    $columns = ['module', 'mib', 'object_type', 'oid', 'syntax', 'description', 'max_access', 'status', 'included_by'];
 ...
 * NET-SNMP-TC::linux
 */
function snmp_mib_translate($oid, $module, $mibdir = null, $device = [])
{
    if ($module !== 'all') {
        $oid = "$module::$oid";
    }
...
function save_mibs($device, $mibname, $oids, $mibdef, &$graphs)
{
    ...
    foreach ($oids as $index => $array) {
        foreach ($array as $obj => $val) {
            ...
            $tags = [
                'rrd_def'       => $rrd_def,
                'rrd_name'      => [$mibname, $mibdef[$obj]['shortname'], $index],
                'rrd_oldname'   => [$mibname, $mibdef[$obj]['object_type'], $index],
                'index'         => $index,
                'oid'           => $mibdef[$obj]['oid'],
                'module'        => $mibdef[$obj]['module'],
                'mib'           => $mibdef[$obj]['mib'],
    ...
    // update database
    $columns = ['device_id', 'oid', 'module', 'mib', 'object_type', 'value', 'numvalue'];
...
function poll_mibs($device, &$graphs)
{
    ...
    foreach (load_device_mibs($device) as $name => $module) {
        echo "$name ";
        d_echo("\n");
        $oids = snmpwalk_cache_oid($device, $name, [], $module, null, '-OQUsb');
        d_echo($oids);
    ...
    foreach ($mibs as $name => $module) {
        ...
        if ($translated) {
            ...
            if (snmp_mib_load($nam, $mod, $included_by, null, $device) > 0) {
                // NOTE: `last_modified` omitted due to being automatically maintained by MySQL
                $columns = ['device_id', 'module', 'mib', 'included_by'];
                $rows = [];
                $rows[] = [
                    'device_id'   => $device['device_id'],
                    'module'      => $mod,
