...
function get_device_graphs($device)
{
    ...
    return dbFetchColumn($query, [$device['device_id']]);
}
...
function get_mibval($device, $oid)
{
    ...
    return dbFetchRow($sql, [$device['device_id'], $oid]);
} // get_mibval
...
function set_user_pref($name, $value, $user_id = null)
{
    ...
    if (dbFetchCell('SELECT count(*) FROM `users_prefs` WHERE `user_id`=? AND `pref`=?', [$user_id, $name])) {
