...
// api-platform drops every BelongsTo foreign key from the property list
// (ModelMetadata::getAttributes), so naming it here is what puts it back.
#[ApiProperty(property: 'location_id')]
...
// Relations whose target is also a resource are serialized as IRIs unless
// they are turned off here. /devices/{id}/ports is the supported way to reach
// a device's ports; parents and children are the dependency tree, which the
// API does not model yet.
#[ApiProperty(property: 'ports', readable: false)]
#[ApiProperty(property: 'parents', readable: false)]
#[ApiProperty(property: 'children', readable: false)]
...
class Device extends BaseModel
{
    ...
    /**
     * Kept out of every serialization of this model, which is what stops the
     * v2 API returning them: api-platform derives a resource's fields from
     * the table schema and honours $hidden (ModelMetadata::attributeIsHidden).
     * Attribute access is unaffected, so SnmpConfig::fromDevice() and the
     * device edit form still read them.
     */
    protected $hidden = [
        'community',
        'authname',
